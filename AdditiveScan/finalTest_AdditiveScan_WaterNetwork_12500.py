...
def additive_graphscan(graph, att, npss,globalPValue, iterations_bound=10, ncores=8, minutes=30):
    ori_graph = graph
    ori_att = att
    alpha_max = 0.15
...
def finalTest_AdditiveScan_multiwCore_Weibo(numOfThreads,resultFileName, iterations_bound, ncores, minutes,rootFolder,controlFileList):
    folders = []
    files = []
    ...
    for fileName in files:
        ...
        for folder in folders:
            if os.path.isfile(rootFolder+'/'+folder+'/'+fileName):
                print 'current processing file : ', os.path.join(fileName)
                start_time = time.time()
                npss = 'BJ'
                graph, att,globalPValue, true_subGraph = read_APDM_data(rootFolder+'/'+folder+'/'+fileName)
                subset_score = additive_graphscan(graph, att,'BJ',globalPValue,iterations_bound,ncores,minutes)
...
def main():
    ...
    rootFolder = '../../realDataSet/WaterNetwork'
    controlFileList = '../../ComparePreRecROC/WaterNetwork_12500/controlFileList.txt'
    finalTest_AdditiveScan_multiwCore_Weibo(numOfThreads,resultFileName, iterations_bound, ncores, minutes,rootFolder,controlFileList)
